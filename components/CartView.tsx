...
export const CartView = ({ cart, onUpdateQuantity, onRemoveItem, onCheckout, onBack }: CartViewProps) => {
            ...
            {cart.map(item => (
              <div key={item.id} className="flex flex-col sm:flex-row items-center justify-between p-3 border rounded-lg">
                ...
                <div className="flex items-center space-x-2 mb-2 sm:mb-0">
                  <button onClick={() => onUpdateQuantity(item.id, item.quantity - 1)} className="w-8 h-8 bg-gray-200 rounded-full font-bold hover:bg-gray-300" aria-label={`減少 ${item.name} 的數量`}>-</button>
                  <span className="w-10 text-center font-semibold">{item.quantity}</span>
                  <button onClick={() => onUpdateQuantity(item.id, item.quantity + 1)} className="w-8 h-8 bg-gray-200 rounded-full font-bold hover:bg-gray-300" aria-label={`增加 ${item.name} 的數量`}>+</button>
                </div>
